...
raion_olympiad = () ->
    page("Про районную/городскую олимпиаду", String.raw"""
        ...
        <p>Последние шесть лет задачи районной олимпиады составляет другое жюри, поэтому в первую очередь смотрите на задачи последних 6 лет, а не более старые.</p>
...
school_olympiad = () ->
    page("Про школьную олимпиаду", String.raw"""
        ...
        <h3>О чем речь?</h3>
        <p>Школьная олимпиада по информатике в Нижнем Новгороде и Нижегородской области состоится, видимо, <b>26 октября</b>, но обязательно уточните сроки заранее в своей школе. (Школьники не из области — тоже уточняйте у себя в школе).</p>
        ...
        <p>Последние годы менялись организаторы и формат олимпиады, поэтому задачи за старые годы особого смысла смотреть нет. На алгопроге есть задачи прошлого года.</p>
...
reg_about = () ->
    page("Про областную (региональную) олимпиаду", String.raw"""<h1>Про областную (региональную) олимпиаду</h1>
        ...
        и итоговые баллы равны первичным, деленным на 8 (т.е. баллы приводятся в диапазон от 0 до 100). Это какие-то бюрократические заморочки, реально все используют «первичные» баллы, но в формальной итоговой таблицу могут указывать «итоговые».</p>
...
label_ochn = () ->
    label("""
        ...
        <h3>ННГУ</h3>
        <p><b>Только для школьников</b></p>
        <p>Возможно, будут занятия по воскресеньям в ННГУ им. Лобачевского, их будет вести другой преподаватель. Следите за объявлениями.</p>
        <hr>""")
