...
NORETURN void InStream::quit(TResult result, const char* msg)
{
    ...
    if (resultName != "")
    {
        ...
        if (appesMode)
        {
            ...
            if (isPartial)
                std::fprintf(resultFile, "<result outcome = \"%s\" pctype = \"%d\" comment=\"", outcomes[(int)_partially].c_str(), pctype);
            else
            {
                if (result != _points)
                    std::fprintf(resultFile, "<result outcome = \"%s\" comment=\"", outcomes[(int)result].c_str());
                else
                {
                    ...
                    std::fprintf(resultFile, "<result outcome = \"%s\" points = \"%s\" comment=\"", outcomes[(int)result].c_str(), stringPoints.c_str());
                }
            }
...
void registerInteraction(int argc, char* argv[])
{
    ...
    if (argc == 6)
    {
        if (strcmp("-APPES", argv[5]) && strcmp("-appes", argv[5])&& strcmp("-xml", argv[5]))
        {
            quit(_fail, std::string("Program must be run with the following arguments: ") +
